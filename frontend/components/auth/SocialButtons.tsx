...
export function SocialButton({ provider, children, icon, variant = 'outline' }: SocialButtonProps) {
  ...
  const handleSocialLogin = async () => {
    ...
    try {
      ...
      // Check for errors
      if (result.error) {
        addToast(result.error.message || `Failed to sign in with ${provider}`, 'error');
        setLoading(false);
        return;
      }
      
      // Success - Better Auth will handle the redirect
      addToast(`Signing in with ${provider}...`, 'success');
    } catch (error: any) {
      console.error(`${provider} login error:`, error);
      addToast(error.message || `Failed to connect to ${provider}`, 'error');
      setLoading(false);
    }
  };
